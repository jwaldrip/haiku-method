...
# Generate a cache-safe hash from a URI string.
# Uses SHA-256 truncated to 8 hex characters.
# Usage: _uri_cache_hash <uri>
# Output: 8-char hex hash
_uri_cache_hash() {
  local uri="$1"
  if command -v shasum >/dev/null 2>&1; then
    printf '%s' "$uri" | shasum -a 256 | cut -c1-8
  elif command -v sha256sum >/dev/null 2>&1; then
    printf '%s' "$uri" | sha256sum | cut -c1-8
  else
    printf '%s' "$uri" | md5 | cut -c1-8
  fi
}
...
# Export Instructions for $provider
...
1. Use \`ToolSearch\` to find: \`$mcp_hint\`
2. Call \`$tool_name\` with:
$(echo "$parameters" | sed 's/^/   /')
3. Save the exported PNG to: \`$output_path\`
EOF
}
...
_resolve_design_ref_field() {
  ...
  if [[ "$design_ref" =~ ^[a-z]+:// ]]; then
    local _uri_provider _uri_path
    _uri_provider=$(_parse_provider_uri "$design_ref" | head -1)
    _uri_path=$(_parse_provider_uri "$design_ref" | tail -1)
...
dlc_resolve_design_ref() {
  ...
  if [ "$_RESOLVED_FORMAT" = "provider-uri" ] || [[ "$_RESOLVED_FORMAT" =~ ^(op|pen|excalidraw|fig)$ ]]; then
    ...
    cache_hash=$(_uri_cache_hash "$_RESOLVED_SOURCE")
