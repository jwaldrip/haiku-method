...
`ai-dlc:hat-testing` - Evaluation-Driven Development for hat definitions.

## Synopsis

```
/hat-testing [hat-name]
```
...
Record the following in `.ai-dlc/hat-tests/{hat-name}/{scenario-slug}.md`:
...
```bash
mkdir -p .ai-dlc/hat-tests/${HAT_NAME}
```
...
7. Commit both the updated hat definition and the hat test

### Step 5: Commit Artifacts

```bash
git add .ai-dlc/hat-tests/${HAT_NAME}/${SCENARIO_SLUG}.md
git commit -m "hat-test(${HAT_NAME}): ${SCENARIO_SLUG}"
```

If hat definition was updated:
```bash
git add .ai-dlc/hats/${HAT_NAME}.md
git commit -m "refine(${HAT_NAME}): close rationalization loophole from hat test"
```
...
### Next Steps
- Run `/hat-testing {hat-name}` again with a different scenario
