...
```bash
INTENT_DIR="$REPO_ROOT/.haiku/${SLUG}"
mkdir -p "$INTENT_DIR"
...
**Discovery:** Written to .haiku/{slug}/discovery.md

### Artifacts Created
| File | Description |
|------|-------------|
| `.haiku/{slug}/intent.md` | Intent definition with domain model |
| `.haiku/{slug}/unit-01-{name}.md` | {brief description} |
| `.haiku/{slug}/unit-02-{name}.md` | {brief description} |
| ... | ... |
| `.haiku/{slug}/discovery.md` | Exploration findings |
| `.haiku/{slug}/operations/{name}.md` | {operation description} |
| ... | ... |
...
- **Open PR**: The adoption artifacts are already committed to the `$ADOPT_BRANCH` branch (created in Phase 6). Push and open a PR against the default branch:
  ...
  **Operations:** ${OP_COUNT}
  ...
  ### Artifacts
  - \`.haiku/${SLUG}/intent.md\`
  - \`.haiku/${SLUG}/unit-NN-{name}.md\` (${UNIT_COUNT} files)
  - \`.haiku/${SLUG}/discovery.md\`
  $([ "${OP_COUNT}" -gt 0 ] && echo "- \`.haiku/${SLUG}/operations/\` (${OP_COUNT} files)")
