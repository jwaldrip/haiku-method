...
Manifests are generated by `/operate --deploy` into the `operations/deploy/` subdirectory using type-specific filenames (`{name}.cronjob.yaml`, `{name}.deployment.yaml`, `{name}.workflow.yaml`, `{name}.compose.yaml`, `{name}.service`/`.timer`).
